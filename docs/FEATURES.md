...
| 2-stage sQ prefetch (persistent) | — (not shipped) | **REJECTED 2026-08-01** (roadmap Phase 2.5). Implemented + bitwise-exact; measured FLAT at all self shapes (the shipped design already releases Q right after the last QK GEMM, so the next-tile prefetch chain is covered by the trailing-PV/finalize/epilogue window; h40/75600 = built-in noise control since the flag is inert under the single scheduler). Not worth +32 KB smem + a pipeline stage for zero. **Implementation lesson recorded:** the Q pipeline has TWO PipelineStates (producer AND consumer) — changing stage count on only one deadlocks at ≥2 tiles/slot (spin at ~116 W; small smoke shapes pass — always smoke multi-tile persistent configs). | ABBA medians 0.9948–0.9987, IQRs straddle 1.0 |
...
| `cluster_n` = 4 | **2** stays | **REJECTED 2026-07-31** (roadmap Phase 2 item 2). Works out of the box (assert widened to allow 4; exact incl. a 3-phantom tail at 1152) but measures 4.7–6.4% SLOWER than cluster_n=2 at ALL self shapes. 4-way lockstep amplifies the pair-coupling jitter the fwd already measured (~1.5% at 2-way long chains), and the multicast TMA boxes shrink to quarter-tiles. The second traffic halving cannot pay for it. | paired ABBA c2/c4 medians: 0.9532/0.9382/0.9405/0.9359 |
| dV-GEMM hoist (dS-bubble probe) | — (not shipped) | **REJECTED 2026-08-01** (roadmap Phase 3, probe #1). Issuing GEMM 3 (dV += P^T@dO, rs) right after the GEMM-2 wait — before the dS pointwise/cvt/StMatrix section — to fill the measured 765–1010 idle-cycle pocket. Operands are ready there and the wait-group accounting is sound, yet it measures **15.5% SLOWER uniformly**: a WGMMA issue mid-pointwise brings a `wgmma.fence`, which serializes the surrounding ALU register traffic — the interleave costs ~4× the bubble it fills. Mechanism note for Phase 3 proper: the pocket is protected against fine-grained reordering; only a coarse-grain restructure that keeps GEMM issues in contiguous bursts (e.g. rotating the next block's S-GEMM into the epilogue region) can attack it. | ABBA medians 0.8427–0.8462 |
